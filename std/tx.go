...
import (
	clientx "github.com/irisnet/service-sdk-go/client/tx"
...
func (tx Transaction) ValidateBasic() error {
	...
	if tx.Fee.Amount.IsAnyNegative() {
		return sdk.Wrapf("invalid fee provided: %s", tx.Fee.Amount)
	}
	if len(stdSigs) == 0 {
		return sdk.Wrapf("no signature")
	}
	if len(stdSigs) != len(tx.GetSigners()) {
		return sdk.Wrapf("wrong number of signers; expected %d, got %d", tx.GetSigners(), len(stdSigs))
	}
